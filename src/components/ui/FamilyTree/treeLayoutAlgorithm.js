import { drawNode,  drawLine, drawMemberNode } from './treeDrawingUtils.js';

const drawSingleSpouse = (svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start) => {
  ...
  const member = members.find(m => m.id === familyTree.id);
  const spouse = members.find(m => m.id === familyTree.family[0].spouse);
  drawMemberNode(svg, member_pos, y_new_pos, maxWidth, spacings.w_node, colors.memberNode, 8, member);
  drawMemberNode(svg, spouse_pos, y_new_pos, maxWidth, spacings.w_node, colors.spouseNode, 8, spouse);
  ...
  familyTree.family[0].children.forEach(child => {
    ...
    if (child.family.length === 1) {
      drawSingleSpouse(svg, child, members, inter_mid, x_mid, y_new_intermediary, spacings, colors, false);
    } else if (child.family.length === 0) {
      if (number_of_children === 1) x_mid = x_mid - child.subtree_len / 2 + familyTree.subtree_len / 2;
      drawSingleMember(svg, child, members, inter_mid, x_mid, y_new_intermediary, spacings, colors, false);
    } else {
      drawMultipleSpouses(svg, child, members, inter_mid, x_mid, y_new_intermediary, spacings, colors, false);
    }
  });
};

const drawMultipleSpouses = (svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start) => {
  ...
  familyTree.family.forEach(family => {
    ...
    const spouse = members.find(m => m.id === family.spouse);
    drawMemberNode(svg, spouse_pos, y_new_pos, maxWidth, spacings.w_node, colors.spouseNode, 8, spouse);
...
const drawSingleMember = (svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start) => {
...
export const drawTree = (svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start) => {
  if (familyTree.family.length === 0){
    drawSingleMember(svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start);
  } else if (familyTree.family.length === 1) {
    drawSingleSpouse(svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start);
  } else {
    drawMultipleSpouses(svg, familyTree, members, inter_mid, x_mid, y_pos, spacings, colors, start);
  }
};
